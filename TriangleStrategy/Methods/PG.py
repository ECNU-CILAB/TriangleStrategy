...
class DecisionMakerPG():
    '''The decision maker of PG.
        ...
        with torch.no_grad():
            ...
            for i in range(input_data.shape[1]):
                ...
                if i < self.head_mask or i >= self.tail_mask:
                    ...
                    action = (torch.ones(seq_num) *
                              2).to(torch.int64).to(self.device)
...
class TrainerPG():
    ...
    def calculate_loss(self, model, decision_maker, input_data_batch, buy_price_batch, sell_price_batch):
        ...
        # prob
        model_output = decision_maker.get_model_output(
            model, input_data_batch, action_batch)
        prob = torch.nn.functional.softmax(model_output, dim=2)
        ...
        reward_baseline = 0
        loss_value = -(reward_batch - reward_baseline)*torch.log(prob_action)
    ...
    def train_epoch(self, model, data_loader, optimizer, **kwargs):
        ...
        Args:
            ...
            decision_maker (DecisionMakerPG): The decision maker that chooses actions according to the model\'s output.
